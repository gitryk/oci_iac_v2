...
resource "oci_core_instance" "this" {
  ...
  compartment_id       = var.compartment_id
  display_name         = "${var.project_name}-vm-${each.value.name}"
  ...
  metadata = {
    ...
    user_data           = sensitive(base64encode(templatefile("./script/cloud_init.sh", { wireguard_conf = var.wg0_conf, os_image = each.value.os_image, install_target = each.key, ts_authkey = var.ts_authkey })))
  }
  ...
  create_vnic_details {
    ...
    display_name              = "${var.project_name}-${each.value.name}-vnic"
  ...
  freeform_tags = merge(var.tagging, {
    "Name" = "${var.project_name}-vm-${each.value.name}"
    "Type" = "Instance"
...
resource "oci_core_route_table" "pri_route" {
  ...
  display_name = "${var.project_name}-route-pri"
  route_rules {
  ...
  freeform_tags = merge(var.tagging, {
    "Name" = "${var.project_name}-route-pri"
    "Type" = "Network"
  })
